...
let parent_clicked_row;
let parent_clicked_col;
let parent_counter_id;

let gbl_table_container_id;
let gbl_json_input_container_id;
let gbl_json_output_container_id;
let gbl_json_to_table_btn_id;
let gbl_table_to_json_btn_id;
...
const obj_clicked_row = {};
const obj_clicked_col = {};
...
function jsonEditorInit(table_container_id, json_input_container_id, json_output_container_id, json_to_table_btn_id, table_to_json_btn_id) {
    ...
    $(`#${json_to_table_btn_id}`).on('click', () => {
        try {
            const value = $(`#${json_input_container_id}`).val().trim();
        ...
        switch (key) {
            ...
            case 'pasteRow':
                ...
                for (i = 0; i < inner_table_count; i++) {
                    const t = inner_tables[i].outerHTML;
...
function makeTable(obj_for_table, counter = 1) {
    const table_html = jQuery.parseHTML(`<table class = "json_table" counter-id = ${counter} id = "json_table_${counter}"></table>`);
    let i = 0;
    const header_names = {};
    const local_counter = counter;
    let td_attr;

    $.each(obj_for_table, (level1_k, level1_v) => {
        $.each(level1_v, (k, v) => {
            ...
            const cell = $(table_html).find(`tr[counter-id="${local_counter}"]`).last().find(`td[counter-id="${local_counter}"]`)
.eq(header_names[k]);
    ...
    $(table_html).find('td').each((td_i, td_v) => {
...
function insertColumn(table_ref, header_name, counter) {
    if (!$(table_ref).find(`tr[counter-id="${counter}"]`).first().length) {
        ...
        const thead = `<thead  counter-id = ${counter} id = "json_table_header_${counter}"><tr counter-id = ${counter}><th counter-id = ${counter} > <div contenteditable=true> ${header_name} </div> </th></tr></thead>`; // counter id at cell level
        const tbody = `<tbody  counter-id = ${counter} id = "json_table_body_${counter}"><tr counter-id = ${counter}><td counter-id = ${counter} ><div contenteditable=true></div></td></tr></tbody>` // counter id at cell level
    ...
    const inserted_td = $(table_ref).find(`tr[counter-id="${counter}"]`).first().find(`td[counter-id="${counter}"]`)
.last();
...
function makeJson(counter = 1) {
    ...
    const row_finder = `#json_table_body_${counter} tr[counter-id=${counter}]`;
    $(row_finder).each((row_i, row_v) => {
        const obj = {};
        // console.log('Outer loop id, value: ', row_i, row_v);

        $(header).each((header_i, header_value) => {
            const cell = $(row_v).children('td').eq(header_i);
            const td_attr = $(cell).attr('td_attr');
            const inner_text = $(cell).children('div').text().trim();
            const inner_table = $(cell).find('table');

            switch (td_attr) {
                case 'value':
                    if (inner_text != '' && inner_text != null) { obj[header_value] = inner_text; }
